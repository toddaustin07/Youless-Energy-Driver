--[[
  ...
  Youless LS120 P1 energy meter driver
...
-- Constants
local DEVICE_PROFILE = 'youless.v1g'
...
local function create_device(driver)
  ...
  local MODEL = 'Youless LS120'
  local VEND_LABEL = 'Youless Energy Meter #' .. tostring(#devices + 1)
  local ID = 'YoulessV1' .. tostring(socket.gettime())
...
local function do_refresh(device)
  ...
  ret, e_response = fetch_device_data(device, "http://" .. device.preferences.deviceaddr .. "/a?f=j")
  --ret, e_response = fetch_device_data(device, "http://" .. device.preferences.deviceaddr .. ":6666/a?f=j")    -- ***** TESTING

  if ret == 'OK' then
  
    if device.preferences.includegas then
      ret, g_response = fetch_device_data(device, "http://" .. device.preferences.deviceaddr .. "/e")
      --ret, g_response = fetch_device_data(device, "http://" .. device.preferences.deviceaddr .. ":6666/e")    -- ***** TESTING
    end

    update_device_data(device, parser.parsedata(device, e_response, g_response))
  end
...
local function discovery_handler(driver, _, should_continue)
  ...
  if not initialized then
    ...
    log.info ('Youless device already created')
  end
end
...
log.info ('Youless Energy Meter v1.0 Started')
